...
} //LM cartographer SLAM 시스템의 상태를 .pbstream파일로 부터 불러옴.
...
int MapBuilderBridge::AddTrajectory(
  ...
  auto emplace_result =
      trajectory_options_.emplace(trajectory_id, trajectory_options); //LM 이미 trajectory_id를 key로 갖는 값이 있으면 추가하지 않음.
      //LM trajectory id 하나에 모든 trajectort_options의 값을 다 집어 넣는다.
  CHECK(emplace_result.second == true); // LM first= 삽입된 위치, second=삽입 성공여부
